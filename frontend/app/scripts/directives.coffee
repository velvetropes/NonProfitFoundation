...
sfDirectives.directive 'videoPlayerModal', ["$window", ($window) ->
  ...
  link: (scope, element, attrs) ->
    ...
    scope.$watch('show', (newVal, oldVal) ->
      if newVal && !oldVal
        scope.iframeContent = newVal.replace(/(?:http(?:s?):\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/(?:watch\?v=)?(.+)/g, '<iframe width="100%" height="100%" src="https://www.youtube.com/embed/$1?autoplay=1" frameborder="0" allowfullscreen></iframe>')
