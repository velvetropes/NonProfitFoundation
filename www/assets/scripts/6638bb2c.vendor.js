function Swipe(container, options) {
    ...
    function setup() {
        ...
        slidePos = new Array(slides.length), width = container.getBoundingClientRect().width || container.offsetWidth, 
    ...
    function circle(index) {
        if (0 > index) for (;0 > index; ) index += slides.length;
        ...
        var interval, isScrolling, delay = options.auto || 0, start = {}, delta = {}, events = {
            ...
            move: function(event) {
                if (!(event.touches.length > 1 || event.scale && 1 !== event.scale)) {
                    ...
                    translate(index, delta.x + slidePos[index], 0), translate(circle(index + 1), delta.x + slidePos[circle(index + 1)], 0)) : (delta.x = delta.x / (!index && delta.x > 0 || index == slides.length - 1 && delta.x < 0 ? Math.abs(delta.x) / width + 1 : 1), 
...
jvm.VMLShapeElement.prototype.applyAttr = function(e, t) {
    ...
    width: 900
}), $.fn.vectorMap("addMap", "world_mill_en2", {
    insets: [ {
        height: 550,
        width: 950,
        top: 0,
...
!function(angular) {
    ...
    angular.module("ng.shims.placeholder", []).directive("placeholder", function() {
        if (!angular.mock) {
            ...
            if (void 0 !== test.placeholder) return {};
        }
        return {
            restrict: "A",
            require: "ngModel",
            ...
            link: function(scope, elem, attrs, ngModel) {
                function setValue(val) {
                    val ? (elem.removeClass("empty"), elem.val(val)) : (elem.addClass("empty"), elem.val(attrs.placeholder), 
                    "password" === orig_type && elem.attr("type", "text"));
                }
                var orig_type = attrs.type;
                elem.val() ? ngModel.$setViewValue(elem.val()) : (elem.addClass("empty"), elem.val(attrs.placeholder), 
                "password" === orig_type && elem.attr("type", "text"), ngModel.$setViewValue("")), 
                elem.bind("focus", function() {
                    elem.hasClass("empty") && (elem.val(""), elem.removeClass("empty error"), elem.attr("type", orig_type));
                }), elem.bind("blur", function() {
                    var orig_val = elem.val();
                    setValue(orig_val), scope.$apply(function() {
...
}(window.angular), angular.module("ahundredyears.pagination", []), angular.module("ahundredyears.pagination").directive("pagination", [ function() {
    return {
        restrict: "E",
