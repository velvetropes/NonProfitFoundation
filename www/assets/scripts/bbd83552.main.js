(function() {
    ...
    sfControllers = angular.module("sfControllers", []), sfControllers.config([ "$sceProvider", function($sceProvider) {
    ...
    } ]), sfControllers.controller("globalCtrl", [ "$scope", function($scope) {
        ...
        $scope.$on("modal:hide", function() {
            ...
            return $scope.showModal = !$scope.showModal, $scope.showModal === !0 ? $scope.videoIframe = url : void 0;
    ...
    } ]), sfControllers.controller("BlogIndexCtrl", [ "$scope", "$window", "Articles", "Pagination", function($scope, $window, Articles, Pagination) {
        var currentPageCollection, isMobile, itemsPerPage;
        return itemsPerPage = 9, $scope.articles = [], $scope.nonFeaturedArticles = [], 
        $scope.articlesForMobile = [], $scope.articleFilters = {
            featured: "false",
            blog_item_category: "",
            year: ""
        }, $scope.windowWidth = $window.innerWidth, $scope.articleCategories = [ {
            name: "All",
            tag: ""
        }, {
            name: "News",
            ...
            tag: "Operation Change"
        }, {
            name: "Hearing Missions",
            tag: "Hearing Missions"
        }, {
            ...
            return $scope.articles = data instanceof Array ? data : [ data ], $scope.pagination = Pagination.getNew(itemsPerPage), 
            $scope.nonFeaturedArticles = _.filter($scope.articles, function(article) {
                return "false" === article.featured;
    ...
    sfDirectives = angular.module("sfDirectives", [ "ngSanitize", "sfFilters" ]), sfDirectives.directive("accordion", [ function() {
        ...
        return {
            ...
            link: function(scope, element) {
                return scope.shares = 0, $http.get("https://api.facebook.com/method/links.getStats?urls=" + scope.url + "&format=json").success(function(res) {
                    return scope.shares = res[0].share_count;
                }).error(function() {
    ...
    } ]), sfDirectives.directive("gallerySlide", [ function() {
        var link, result;
        return link = function(scope) {
        ...
        return link = function(scope, element) {
            ...
            return config = {
                ...
                return scope.pane = $(".thumblist-nav"), scope.pane.jScrollPane(config);
            }, 1400), scope.isFullHeight = function() {
                var _ref;
                return (null != (_ref = scope.full) ? _ref.length : void 0) > 0 && "true" === scope.full;
        ...
        return {
            ...
            link: function(scope, element, attrs) {
                ...
                scope.hideModal = function() {
                    ...
                    return newVal && !oldVal ? (scope.iframeContent = newVal.replace(/(?:http:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/(?:watch\?v=)?(.+)/g, '<iframe width="100%" height="100%" src="http://www.youtube.com/embed/$1?autoplay=1" frameborder="0" allowfullscreen></iframe>'), 
        ...
        return {
            ...
            link: function(scope) {
                ...
                return createImagePattern = function(id, url) {
                    ...
                    image.setAttribute("x", "0"), image.setAttribute("y", "0"), image.setAttribute("width", "30"), 
                    image.setAttribute("height", "30"), image.setAttributeNS(svgNSXLink, "xlink:href", url), 
                    svgMap.appendChild(pattern), void pattern.appendChild(image);
                }, generateMap = function() {
                    var i, icon, _i, _len, _ref;
                    for (console.log(scope), $("#world-map-gdp").vectorMap({
                        map: "world_mill_en",
                        ...
                        onMarkerClick: function(event, index) {
                            var $popup, content;
                            return content = scope.markers.meta_data[index], $popup = $("#map-popup"), $popup.fadeOut("slow", function() {
                    ...
                    }), _ref = scope.markers.icons, i = _i = 0, _len = _ref.length; _len > _i; i = ++_i) icon = _ref[i], 
    ...
    var HomePageApp, blogPagesApp, galaPageApp, legalPagesApp, mediaMentionsPagesApp, missionsPageApp, previewPageApp, programsPageApp, requiredModules, takeActionPagesApp;
    requiredModules = [ "ngRoute", "ngAnimate", "ngSanitize", "truncate", "sfControllers", "sfDirectives", "sfFilters", "sfServices" ], 
    blogPagesApp = angular.module("blogPagesApp", requiredModules), blogPagesApp.run(function($rootScope, $location, $anchorScroll) {
    ...
    } ]), galaPageApp = angular.module("galaPageApp", requiredModules), galaPageApp.config([ "$routeProvider", function($routeProvider) {
    ...
    } ]), legalPagesApp = angular.module("legalPagesApp", requiredModules), mediaMentionsPagesApp = angular.module("mediaMentionsPagesApp", requiredModules), 
    ...
    } ]), missionsPageApp = angular.module("missionsPageApp", requiredModules), missionsPageApp.run(function($rootScope, $location, $anchorScroll) {
        return $rootScope.$on("$routeChangeSuccess", function() {
            return $anchorScroll();
        });
    ...
    } ]), programsPageApp = angular.module("programsPageApp", requiredModules), programsPageApp.config([ "$routeProvider", function($routeProvider) {
    ...
    }), previewPageApp = angular.module("previewPageApp", requiredModules), previewPageApp.config([ "$routeProvider", function($routeProvider) {
